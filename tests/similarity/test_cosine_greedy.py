import numpy as np
import pytest
from cudams.similarity import CudaCosineGreedy, CPUParallelCosineGreedy
...
def test_cpu_parallel_cosine_greedy_pair(peaks, tolerance, mz_power, intensity_power, expected_matches):
    builder = SpectrumBuilder()
    spectrum_1 = builder.with_mz(peaks[0][0]).with_intensities(peaks[0][1]).build()
    spectrum_2 = builder.with_mz(peaks[1][0]).with_intensities(peaks[1][1]).build()

    cosine_greedy = CPUParallelCosineGreedy(batch_size=8, tolerance=tolerance, mz_power=mz_power, intensity_power=intensity_power)
    score = cosine_greedy.pair(spectrum_1, spectrum_2)
